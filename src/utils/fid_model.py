...
def load_fidnet_v3(dataset_name, num_classes, device: torch.device) -> nn.Module:
    prefix = f"{dataset_name}-max25"
    ckpt_path = os.path.join('./pretrained', prefix, "model_best.pth.tar")
    fid_model = FIDNetV3(
        num_label=num_classes, max_bbox=25
    ).to(device)
